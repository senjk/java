...
public class AccountServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("1".equals(flags)){
			...
			flag=iInsertservice.insert(account);
		}
		if("2".equals(flags)){
			flag=iInsertservice.delete(account);
		}
		if("3".equals(flags)){
			account.setCode(request.getParameter("code"));
			account.setName(request.getParameter("name"));
			...
			account.setSex(request.getParameter("sex"));
			account.setAge(request.getParameter("age"));
			flag=iInsertservice.update(account);
		}
		if(flag){
			List<Account> listAccount =iSelect.listAccount(new Account());
